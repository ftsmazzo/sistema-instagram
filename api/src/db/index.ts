...
`);
  await p.query(`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS crm_cadencia_config jsonb NOT NULL DEFAULT '{"ativo":true,"horas_sem_resposta":24,"alerta_consultor_horas":12,"etapas":[{"horas_apos_parada":24,"mensagem":"Oi {nome}! Passando para saber se ainda posso te ajudar com {objetivo}."},{"horas_apos_parada":72,"mensagem":"Oi {nome}, nossa conversa sobre {objetivo} ficou pendente. Ainda faz sentido retomarmos?"},{"horas_apos_parada":168,"mensagem":"Último contato, {nome}. Se ainda tiver interesse em {objetivo}, é só responder."}]}'::jsonb`);
  await p.query(`ALTER TABLE leads ADD COLUMN IF NOT EXISTS crm_cadencia_serie_id uuid`);
  await p.query(`ALTER TABLE leads ADD COLUMN IF NOT EXISTS crm_cadencia_pausada boolean NOT NULL DEFAULT false`);
  await p.query(`ALTER TABLE leads ADD COLUMN IF NOT EXISTS handoff_alerta_em timestamptz`);
  await p.query(`ALTER TABLE crm_followup_mensagens ADD COLUMN IF NOT EXISTS serie_id uuid`);
  await p.query(`ALTER TABLE crm_followup_mensagens ADD COLUMN IF NOT EXISTS etapa smallint`);
